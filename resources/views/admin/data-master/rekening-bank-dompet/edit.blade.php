@extends('layout.app_admin',['title_satu'=>'Form Edit','title_dua'=>'Rek. Bank/E-Dompet'])
...
@endif
<form action="{{ route('admin.data-master.rekening-bank-dompet.update',$rek_bank_dompet->id) }}" method="POST" class="card">
    @csrf
    @method('put')
    <div class="card-body">
        <div class="row">
            <div class="col-lg-4 mb-3">
                ...
                <select name="bank_dompet" class="form-select @error('bank_dompet') is-invalid @enderror" id="bank-dompet" data-placeholder="pilih bank/e-dompet">
                    ...
                    <option value="{{ $bd->id }}" @if(@old('bank_dompet',$rek_bank_dompet->bank_dompet_id) == $bd->id) selected @endif>{{ $bd->nama }}</option>
                    @endforeach
                </select>
            ...
            <div class="col-lg-4 mb-3">
                ...
                <select name="pemilik" class="form-select @error('pemilik') is-invalid @enderror" id="pemilik" data-placeholder="pilih pemilik">
                    ...
                    <option value="{{ $p->id }}" @if(@old('pemilik',$rek_bank_dompet->user_id) == $p->id) selected @endif>{{ $p->nama_lengkap }}</option>
                    @endforeach
                </select>
            ...
            <div class="col-lg-4 mb-3">
                ...
                <div>
                    <input type="text" name="nomor_rekening" class="form-control @error('nomor_rekening') is-invalid @enderror" value="{{ @old('nomor_rekening',$rek_bank_dompet->nomor_rekening) }}" placeholder="Nomor Rekening..">
