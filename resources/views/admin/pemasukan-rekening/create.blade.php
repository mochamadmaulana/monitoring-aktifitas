@extends('layout.app_admin',['title_satu'=>'Form Tambah','title_dua'=>'Pemasukan Rekening'])
...
@endif
<form action="{{ route('admin.pemasukan.rekening.store') }}" method="POST" class="card">
    @csrf
    <div class="card-body">
        <div class="row">
            <div class="col-lg-3 mb-3">
                ...
                <select name="rekening_debit" class="form-select @error('rekening_debit') is-invalid @enderror" id="rekening-debit" data-placeholder="pilih rekening debit">
                    ...
                    <option value="{{ $rbd->id }}" @if(@old('rekening_debit') == $rbd->id) selected @endif>{{ $rbd->nomor_rekening }} - {{ $rbd->nama }}</option>
                    @endforeach
                </select>
            </div>
            <div class="col-lg-3 mb-3">
                ...
                <div>
                    <input type="text" name="rekening_kredit" class="form-control @error('rekening_kredit') is-invalid @enderror" value="{{ @old('rekening_kredit') }}" placeholder="Rekening Kredit..">
                </div>
            </div>
            <div class="col-lg-3 mb-3">
            ...
            <div class="col-lg-12 mb-3">
                <div class="col-lg-6">
                    ...
                    <div>
                        <textarea name="deskripsi" class="form-control @error('deskripsi') is-invalid @enderror" cols="5" rows="5" placeholder="Untuk uang jajan selama 1 bulan...">{{ @old('deskripsi') }}</textarea>
                    </div>
                </div>
            </div>
        </div>
